...
export const useAdditionalRoutesStore = create<AdditionalRoutesStore>(
  (set) => ({
    selectedEmployeeIds: [],
    selectedAreas: [],
    ...
    addEmployee: (employeeId) => {
      set((state) => {
        ...
        if (state.selectedEmployeeIds.some((id) => Number(id) === numericId))
          return state;
        return {
    ...
    toggleEmployee: (employeeId: number | string) => {
      set((state) => {
        const numericId = Number(employeeId);
        const isSelected = state.selectedEmployeeIds.some(
