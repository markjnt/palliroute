...
export function useFallbackSelectedWeekday() {
  ...
  const setSelectedWeekday = useWeekdayStore(
    (state) => state.setSelectedWeekday,
  );
  ...
  useEffect(() => {
    ...
    return useWeekdayStore.persist.onFinishHydration(() =>
      setWeekdayHydrated(true),
    );
  }, [weekdayHydrated]);

  useEffect(() => {
    if (!weekdayHydrated || !selectedUserId || selectedCalendarWeek == null)
      return;
    if (!isFetched || isError || !holidaysFetched) return;
    if (
      !routesReadyForSelectedWeek(allRoutes, selectedCalendarWeek, isFetching)
    )
      return;

    const weekRoutes = allRoutes.filter(
      (route) =>
        route.calendar_week == null ||
        route.calendar_week === selectedCalendarWeek,
    );
    const nextWeekday = resolveWeekdayAfterWeekChange({
      selectedWeekday,
      today: getCurrentWeekday() as Weekday,
      isAwDay: (day) =>
        isAwCalendarDay(day, getHolidayName(day, selectedCalendarWeek)),
