...
const weekendAw = (weekday: Weekday) =>
...
describe("routesReadyForSelectedWeek", () => {
  it("is ready once any route belongs to the selected week", () => {
    expect(routesReadyForSelectedWeek([{ calendar_week: 34 }], 34, true)).toBe(
      true,
    );
  });

  it("waits while cached routes still belong to another week", () => {
    expect(routesReadyForSelectedWeek([{ calendar_week: 33 }], 34, true)).toBe(
      false,
    );
    expect(routesReadyForSelectedWeek([{ calendar_week: 33 }], 34, false)).toBe(
      false,
    );
  });
