...
export const EmployeeCapacityCard: React.FC<EmployeeCapacityCardProps> = ({
  ...
  const activeCapacities = capacities.filter(
    (cap) => cap.max_count > 0 || (cap.assigned ?? 0) > 0,
  );
  ...
  const getDutyIcon = (capacityType: string) => {
    if (capacityType.includes("AW"))
      return <AWIcon sx={{ fontSize: "0.85rem" }} />;
    if (capacityType.includes("DOCTORS"))
      return <DoctorIcon sx={{ fontSize: "0.85rem" }} />;
    if (capacityType.includes("WEEKEND"))
    ...
    >
      ...
      {activeCapacities.map((capacity) => {
        ...
        const remaining = Math.max(
          0,
          capacity.remaining ?? calculatedRemaining,
        );
        ...
        const isOverCapacity =
          assigned > capacity.max_count ||
          (capacity.max_count === 0 && assigned > 0);
        const isAtLimit =
          !isOverCapacity &&
          capacity.max_count > 0 &&
          assigned === capacity.max_count;
