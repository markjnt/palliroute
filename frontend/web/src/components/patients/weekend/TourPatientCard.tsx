...
export const TourPatientCard: React.FC<TourPatientCardProps> = ({
  ...
  const selectedDayAppointment = appointments.find(
  ...
  const getBgColor = () => {
    switch (visitType) {
      ...
      default:
        return "rgba(158, 158, 158, 0.08)"; // Light gray
    }
  };
  ...
  const handleMoveToArea = async (targetArea: string) => {
    ...
    try {
      if (!selectedDayAppointment) {
        setNotification(
          "Kein Termin für den ausgewählten Tag gefunden",
          "error",
        );
        return;
      }
      ...
      handleAreaMenuClose();
      setNotification(
        `Patient erfolgreich zu ${targetArea} zugewiesen`,
        "success",
      );
    } catch (error) {
