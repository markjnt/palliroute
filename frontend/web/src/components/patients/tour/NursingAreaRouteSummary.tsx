...
function routeForEmployee(
  ...
  return routes.find((r) => r.employee_id === employeeId && r.weekday === weekday.toLowerCase());
}
...
function aggregateForArea(
  ...
  const utilizationPct = sumTarget > 0 ? (sumDuration / sumTarget) * 100 : undefined;
...
export const NursingAreaRouteSummary: React.FC<NursingAreaRouteSummaryProps> = ({
  ...
  const Cell = ({ row }: { row: AreaAgg }) => (
    <Paper
      ...
      sx={(theme) => {
        const main =
          row.region === 'nord' ? theme.palette.primary.main : theme.palette.secondary.main;
        return {
          flex: 1,
          minWidth: 140,
          p: 1.5,
        ...
        <Tooltip title={UTILIZATION_FILTER_TOOLTIP} arrow>
          ...
          >
            ...
            <Typography
              ...
              component="span"
              sx={{
                color: hasTours && row.utilizationPct !== undefined ? uc : 'text.secondary',
                fontWeight: hasTours && row.utilizationPct !== undefined ? 'bold' : 'normal',
              }}
            >
