...
export const useAppointmentManagement = ({
  ...
  const { selectedCalendarWeek, getCurrentCalendarWeek } =
  ...
  // Move single appointment
  const moveAppointmentHandler = useCallback(
    async (params: {
      appointmentId: number;
      sourceEmployeeId?: number;
      targetEmployeeId?: number;
  ...
  // Check for replacement
  const checkReplacementHandler = useCallback(
    async (
      targetEmployeeId: number,
      weekday: string,
      calendarWeek?: number,
      ...
      try {
        const currentWeek =
          calendarWeek || selectedCalendarWeek || getCurrentCalendarWeek();
  ...
  const assignTourAreaHandler = useCallback(
    async (appointmentId: number, targetArea: "Nord" | "Mitte" | "Süd") => {
      try {
        ...
        setNotification(
          "Fehler beim Zuweisen des Tourbereichs (Nord/Mitte/Süd)",
          "error",
        );
        throw error;
      } finally {
