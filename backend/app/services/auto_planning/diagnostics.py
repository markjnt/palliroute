...
def collect_infeasibility_hints(
    ...
    *,
    ...
    fixed_enforced = [
        (e_idx, s_idx) for (e_idx, s_idx) in ctx.fixed_assignments if (e_idx, s_idx) in x_keys
    ]
    ...
    for (d, role), s_indices in sorted(
        shifts_by_day_role.items(), key=lambda x: (x[0][0], x[0][1])
    ):
    ...
    grouped_aw: dict[tuple[int, str], dict[str, list[int]]] = defaultdict(
        lambda: {"sat": [], "sun": []}
    )
    grouped_rb: dict[tuple[int, str, str], dict[str, list[int]]] = defaultdict(
        lambda: {"sat": [], "sun": []}
    )
    for s in shifts:
        if not (pm_start <= s.date <= pm_end):
    ...
    for (cw, area, tod), pair in sorted(
        grouped_rb.items(), key=lambda x: (x[0][0], x[0][1] or "", x[0][2] or "")
    ):
        sat_indices, sun_indices = pair["sat"], pair["sun"]
        if not sat_indices or not sun_indices:
            continue
