            ...
            [
                {
                    ...
                    if ass.shift_instance and ass.shift_instance.shift_definition
                    else None,
                }
    ...
    try:
        ...
        if "shift_instance_id" in data and "employee_id" in data:
            ...
            assignment = Assignment(
                employee_id=employee_id, shift_instance_id=shift_instance_id, source=source
            )
        ...
        # Option 2: Provide shift_definition_id + date + employee_id
            ...
            assignment = Assignment(
                employee_id=employee_id, shift_instance_id=shift_instance.id, source=source
            )
...
def reset_planning():
    ...
    try:
        ...
        assignments_to_delete = (
            ...
            .join(ShiftInstance)
            .filter(ShiftInstance.date >= start_date, ShiftInstance.date <= end_date)
            .all()
        )
...
def _parse_time_account_as_of_from_filename(filename):
    ...
    m = re.search(
        r"\(\s*(\d{1,2})\.(\d{1,2})\.(\d{4})\s*-\s*(\d{1,2})\.(\d{1,2})\.(\d{4})\s*\)", filename
    )
    if not m:
        return None
...
def time_accounts_upload():
    ...
    try:
        ...
        if not as_of:
            return jsonify(
                {
                    "error": "Stand-Datum konnte nicht aus dem Dateinamen gelesen werden (Format: Auswertung (DD.MM.YYYY - DD.MM.YYYY) ...)"
                }
            ), 400
